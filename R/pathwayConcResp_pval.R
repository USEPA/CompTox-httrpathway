...
#'   output.
#' @export
pathwayConcResp_pval <- function(pathset="bhrr",
                                   dataset="arer",
                                   method="fc",
                                   nullset = "arer_RAND1000",
                                   mc.cores=1,
                                   to.file=T,
                                   do.plot = F,
                                   pval = .05,
  ...
  pathscoremat = setDT(pathscoremat)[, list(conc = list(conc),resp = list(pathway_score), size = min(size)), 
                                     by = list(sample_id, dsstox_substance_id, casrn, name, time, pathway, bmed, cutoff, onesd)]
  ...
  #construct PATHWAY_CR
 	...
 	file <- "input/all_pathways_annotated.xlsx"
    ...
    foldname = paste0("output/pathway_conc_resp_plots/",pathset,"_",dataset,"_",method,"_", pval, nametag)
 	  ...
 	  if(mc.cores > 1){
   	  ...
   	  output = parLapply(cl = cl, X=as.list(pnames), fun=plotouter,
   	                         PATHWAY_CR = PATHWAY_CR, foldname = foldname)
 	  } else {
 	    output = lapply(X=as.list(pnames), plotouter,PATHWAY_CR = PATHWAY_CR, foldname = foldname)
 	  }
...
#' @return No output.
#' @export
plotouter = function(proper_name, PATHWAY_CR, foldname){
  #open pdf for plots
  ...
  subframe = subframe[order(-subframe$hitcall, subframe$bmd10),] #order by potency (optional)

  #cycle through pathways (rows) and run pathwayConcRespPlot
  for(i in 1:nrow(subframe)){
    pathwayConcRespPlot(subframe[i,])
  }
