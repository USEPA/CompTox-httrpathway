...
#'
#--------------------------------------------------------------------------------------
noiseHunter.pg.dmso <- function(to.file=F,
                                do.read=F,
  ...
  if(do.1) {
    if(to.file) {
      fname <- paste0(dirout,"DMSO principal components 1.pdf")
    ...
    for(pg in pg.list) {
      ...
      res <- reshape2::dcast(mat,sample_id~probe_id,value.var="probe_count",fill=0.5)
      rownames(res) <- res[,1]
      res <- res[,2:ncol(res)]
      res <- as.matrix(res)
      rs <- rowSums(res)
      ...
      pres <- prcomp(res)
      pc <- pres$x
      plot(pc[,"PC2"]~pc[,"PC1"],xlim=c(-200,200),ylim=c(-200,200),main=paste("PG:",pg),cex.lab=1.2,cex.axis=1.2)
      plot(pc[,"PC3"]~pc[,"PC2"],xlim=c(-200,200),ylim=c(-200,200),main=paste("PG:",pg),cex.lab=1.2,cex.axis=1.2)
      if(!to.file) browser()
    }
    if(to.file) dev.off()
  }
