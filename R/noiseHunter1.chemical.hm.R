#--------------------------------------------------------------------------------------
#' Make heatmaps of the FCMAT and FCMAT.SE by chemical
#'
...
#' @export
#--------------------------------------------------------------------------------------
noiseHunter1.chemical.hm <- function(to.file=F,
                                     do.load=F,
  ...
  if(to.file) {
    fname <- paste0("../output/noiseHunter/noiseHunter1 chemical ",cutoff," heatmap.pdf")
    if(ngene>0) fname <- paste0("../output/noiseHunter/noiseHunter1 chemical ",cutoff," heatmap ngene ",ngene,".pdf")
  ...
  cmap <- cmap[order(cmap$name),]
  cmap <- cmap[order(cmap$pg_id),]
  name.list <- unique(cmap$name)
  cutoff.l2fcfc <- 2
  cutoff.se <- 0.75
  if(ngene== -1) ngene <- ncol(FCMAT2)
  for(i in 1:length(name.list)) {
    name  <- name.list[i]
    cat("Chemical",name,i,length(name.list),"\n")
    sk.list <- cmap[cmap$name==name,"sample_key"]
    pg.list <- cmap[cmap$name==name,"pg_id"]
    pg <- pg.list[1]
    col.list <- gc$color
