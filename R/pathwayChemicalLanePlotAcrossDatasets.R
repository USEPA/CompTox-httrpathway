...
pathwayChemicalLanePlotAcrossDatasets <- function(to.file=F,
  ...
  if(to.file) {
    fname <- paste0("../output/pathway_conc_resp_laneplots/pathwayChemicalLanePlotAcrossDatasets_",chemical.target,"_",pathway.super_class,".pdf")
  ...
  for(pathway in pathway.list) {
    ...
    temp1 <- mat[is.element(mat$pathway,pathway),]
    counter <- 0
