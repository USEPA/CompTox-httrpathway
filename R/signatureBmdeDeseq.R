#--------------------------------------------------------------------------------------
#'
#' Compare the signature PODs between BMDExpress and DESeq2 / TCPL
#' @param to.file If TRUE, write plots to a file
#--------------------------------------------------------------------------------------
signatureBmdeDeseq <- function(to.file=F,
  ...
  if(to.file) {
    fname <- paste0("../output/pod_laneplot/signatureBmdeDeseq_",dataset,"_",pathset,"_",method,".pdf")
  ...
  for(dtxsid in dtxsid.list) {
    ...
    bmds <- all_signature_bmds[is.element(all_signature_bmds$chem_name,name),]
    deseq <- deseq.all[is.element(deseq.all$dtxsid,dtxsid),]

      path.list <- sort(unique(bmds$Pathway))
    path.list <- path.list[is.element(path.list,deseq$signature)]
    rownames(bmds) <- bmds$Pathway
    rownames(deseq) <- deseq$signature
    ...
    temp <- deseq[,c("dtxsid","casrn","name","signature","signature_class","bmdl","bmd","bmdu")]
    temp <- cbind(temp,bmds[,c("chem_name","Pathway","BMDL","BMD","BMDU")])
    res <- rbind(res,temp)
    if(!to.file) browser()
  }
  file <- paste0("../output/pod_laneplot/signatureBmdeDeseq_",dataset,"_",pathset,"_",method,".xlsx")
  write.xlsx(res,file)
  if(to.file) dev.off()
}
