...
#--------------------------------------------------------------------------------------
signatureRank.DESEQ2 <- function(to.file=F,
  ...
  if(to.file) {
    fname <- paste0("../output/signature_rank/signature_rank_",dataset1,"_",dataset2,"_",pathset,"_",method,".pdf")
  ...
  file <- paste0("../output/signature_conc_resp_summary/PATHWAY_CR_",pathset,"_",dataset1,"_",method,"_0.05_conthits.RData")
  print(file)
  load(file)
  deseq1 <- PATHWAY_CR
  ...
  file <- paste0("../output/signature_conc_resp_summary/PATHWAY_CR_",pathset,"_",dataset2,"_",method,"_0.05_conthits.RData")
  print(file)
  load(file)
  deseq2 <- PATHWAY_CR
  ...
  name.list <- c("dtxsid","casrn","name","target_key",
                 "deseq1.first.signature","deseq1.first.signature.score","deseq1.first.signature.class",
                 "deseq1.first.ontarget.signature","deseq1.first.ontarget.signature.rank",
                 "deseq1.first.ontarget.signature.score","deseq1.first.ontarget.signature.pod",
                 "deseq1.first.ontarget.signature.top_over_cutoff",
                 "deseq1.signature.hits",

                 "deseq2.first.signature","deseq2.first.signature.score","deseq2.first.signature.class",
                 "deseq2.first.ontarget.signature","deseq2.first.ontarget.signature.rank",
                 "deseq2.first.ontarget.signature.score","deseq2.first.ontarget.signature.pod",
                 "deseq2.first.ontarget.signature.top_over_cutoff",
                 "deseq2.signature.hits"
                 )
  ...
  for(dtxsid in dtxsid.list) {
    ...
    result[dtxsid,"deseq1.signature.hits"] <- nrow(temp[temp$hitcall>cutoff,])
    pathset.list <- NULL
    ...
    if(!is.null(pathset.list)) {
      ...
      result[dtxsid,"deseq1.first.signature"] <- temp[1,"signature"]
      result[dtxsid,"deseq1.first.signature.score"] <- temp[1,"drank"]
      result[dtxsid,"deseq1.first.signature.class"] <- temp[1,"signature_class"]
      index <- min(which(mask==1))

      result[dtxsid,"deseq1.first.ontarget.signature"] <- temp[index,"signature"]
      result[dtxsid,"deseq1.first.ontarget.signature.rank"] <- index
      result[dtxsid,"deseq1.first.ontarget.signature.score"] <- temp[index,"drank"]
      result[dtxsid,"deseq1.first.ontarget.signature.pod"] <- temp[index,"bmd"]
      result[dtxsid,"deseq1.first.ontarget.signature.top_over_cutoff"] <- temp[index,"top_over_cutoff"]
    }
    ...
    result[dtxsid,"target_key"] <- target_key
    cat(result[dtxsid,"name"],":",target_key,"\n")
    result[dtxsid,"deseq2.signature.hits"] <- nrow(temp[temp$hitcall>cutoff,])
    ...
    if(!is.null(pathset.list)) {
      ...
      result[dtxsid,"deseq2.first.ontarget.signature"] <- temp[index,"signature"]
      result[dtxsid,"deseq2.first.ontarget.signature.rank"] <- index
      result[dtxsid,"deseq2.first.ontarget.signature.score"] <- temp[index,"drank"]
      result[dtxsid,"deseq2.first.ontarget.signature.pod"] <- temp[index,"bmd"]
      result[dtxsid,"deseq2.first.ontarget.signature.top_over_cutoff"] <- temp[index,"top_over_cutoff"]
  ...
  for(i in 1:nrow(result)) {
    if(!is.na(result[i,"deseq1.first.ontarget.signature.rank"]) && !is.na(result[i,"deseq2.first.ontarget.signature.rank"])) {
      x <- result[i,"deseq1.first.ontarget.signature.rank"]
      x <- x+rnorm(1,0,x*0.05)
      y <- result[i,"deseq2.first.ontarget.signature.rank"]
  ...
  for(i in 1:nrow(result)) {
    if(!is.na(result[i,"deseq1.first.ontarget.signature.pod"]) && !is.na(result[i,"deseq2.first.ontarget.signature.pod"])) {
      x <- result[i,"deseq1.first.ontarget.signature.pod"]
      y <- result[i,"deseq2.first.ontarget.signature.pod"]
      target_key <- result[i,"target_key"]
      if(target_key=="thyroid") pathset.list <- c("thyroid")
      if(target_key=="ER") pathset.list <- c("estrogen","Tamoxifen","Cyproterone")
      if(target_key=="ion channel") pathset.list <- c("ion channel","Amiodarone")
  ...
  for(i in 1:nrow(result)) {
    if(!is.na(result[i,"deseq1.first.ontarget.signature.pod"]) && !is.na(result[i,"deseq2.first.ontarget.signature.pod"])) {
      x <- result[i,"deseq1.first.ontarget.signature.top_over_cutoff"]
      y <- result[i,"deseq2.first.ontarget.signature.top_over_cutoff"]
      target_key <- result[i,"target_key"]
      if(target_key=="thyroid") pathset.list <- c("thyroid")
      if(target_key=="ER") pathset.list <- c("estrogen","Tamoxifen","Cyproterone")
      if(target_key=="ion channel") pathset.list <- c("ion channel","Amiodarone")
  ...
  for(i in 1:nrow(result)) {
    if(!is.na(result[i,"deseq1.first.ontarget.signature.pod"]) && !is.na(result[i,"deseq2.first.ontarget.signature.pod"])) {
      x <- result[i,"deseq1.signature.hits"]
      y <- result[i,"deseq2.signature.hits"]
      target_key <- result[i,"target_key"]
      if(target_key=="thyroid") pathset.list <- c("thyroid")
      if(target_key=="ER") pathset.list <- c("estrogen","Tamoxifen","Cyproterone")
      if(target_key=="ion channel") pathset.list <- c("ion channel","Amiodarone")
