...
#' @return No output.
#' @export
pathwayScoreCoreGSVA <- function(sk.list,
                                   pathset="FILTERED",
                                   dataset,
                                   fcmat,
                                   chem_dict,
  ...
  #call gsva: converts gene by sample matrix to pathway by sample matrix
  tryCatch({
    res <- gsva(expr=t(data),
                gset.idx.list=pathway_data,
                method="gsva",
                min.sz=1,
                max.sz=Inf,
  ...
  trimpdata = lapply(pathway_data, function(x){x[x %in% colnames(fcmat)]}) #remove genes not in fcmat from pathway_data
  sizemat = sapply(trimpdata, function(x){rowSums(notnamat[,x, drop = F])}) #count nonmissings and out skey x pathway matrix
