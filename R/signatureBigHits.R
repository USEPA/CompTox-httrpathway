...
#--------------------------------------------------------------------------------------
#' Compile the big hits
#'
#' heparg2d_toxcast_pfas_pe1_normal
#' mcf7_ph1_pe1_normal_good_pg
#' u2os_toxcast_pfas_pe1_normal
#--------------------------------------------------------------------------------------
signatureBigHits <- function(to.file=F,
                             do.load=F,
  ...
  mat = mat[mat$bmd<1,]
  mat = mat[mat$top_over_cutoff>10,]
  ...
  cat("Chemicals: ",length(unique(mat$name)),"\n")
  cat("Signatgures: ",length(unique(mat$signature)),"\n")

  if(to.file) {
    fname <- paste0("../output/signature_bighits/",celltype,"/signature_bighits_",celltype,
  ...
  #######################################################################################
  res = reshape2::dcast(mat,name~signature,fill=0,value.var="pot",fun.aggregate=mean)
  rownames(res) = res[,1]
  ...
  result <- heatmap.2(res,
                      ...
                      key.xlab="AUC",
                      cex.main=1,
                      main=paste(celltype,"x signature"))


  if(!to.file) browser()
  #######################################################################################
  res = reshape2::dcast(mat,name~super_target,fill=0,value.var="pot",fun.aggregate=mean)
  rownames(res) = res[,1]
  res = as.matrix(res[,2:ncol(res)])
  res1 = res
  res1[res1>0] = 1
  ...
  result <- heatmap.2(res,
                      ...
                      key.xlab="AUC",
                      cex.main=1,
                      main=paste(celltype,"x super target"))
