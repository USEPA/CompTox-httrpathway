...
tc.trend.1 <- function(to.file=F,
  ...
  if(to.file) {
    fname <- paste0("../output/ tc.trend.1 ",dataset,"_",sigset,".pdf")
  ...
  #plot(bmd~tc,log="y",pch=".")

  par(mfrow=c(3,1),mar=c(4,4,2,2))
  vals <- c(1e-4,1e-3,1e-2,1e-1,1,10,100,1000)
  nval <- length(vals)
  x <- NULL
  y <- NULL
  for(val in vals) {
  ...
  for(i in 1:length(vals)) {
    ...
    bmd <- mat2$bmd
   # plot(bmd~tc,log="y",pch=".",main=paste("cutoff=",tccut))
    ...
    res[i,"targets"] <- n2
    #if(tccut==4) {
    #  temp <- unique (mat2[,c("signature","super_target")])
    #  browser()
    #}
  #  if(!to.file) browser()
  }
  file <- paste0("../output/ tc.trend.1 ",dataset,"_",sigset,".xlsx")
  write.xlsx(res,file)
...
##################################################################################
  ...
  cs <- colSums(res)
  res <- res[,cs>0]
  result <- heatmap.2(as.matrix(t(res)),
                      margins=c(10,10),
                      dendrogram="both",
                      scale="none",
                      main="",
  ...
  file <- paste0("../output/ tc.trend.1 ",dataset,"_",sigset," chem target.xlsx")
  write.xlsx(res,file,rowNames=T)


  ##################################################################################
  file <- "../input/PFAS 149 2019-08-14 chemicals.xlsx"
  pfas <- read.xlsx(file)
  dtxsid.list <- pfas$dtxsid
  ...
  result <- heatmap.2(as.matrix(t(res)),
                      ...
                      key.xlab="target count",
                      cex.main=1)
  file <- paste0("../output/ tc.trend.1 ",dataset,"_",sigset," chem target pfas.xlsx")
  write.xlsx(res,file,rowNames=T)
