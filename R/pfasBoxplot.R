#--------------------------------------------------------------------------------------
#' Generate chemicalwise boxplot of the PFAS BMD distributions by super_target
#'
#' heparg2d_toxcast_pfas_pe1_normal
#' u2os_toxcast_pfas_pe1_normal
#'
#--------------------------------------------------------------------------------------
pfasBoxplot <- function(to.file=F,
                        do.load=F,
                        dataset="PFAS_U2OS",
                        sigset="screen_large",
                        method="fc",
  ...
  if(to.file) {
    fname <- paste0("../output/signature_pfas/",celltype,"/pfas_boxplot_",dataset,"_",sigset,"_",method,".pdf")
  ...
  for(chem in chem.list) {
    ...
    if(length(y)>0 && nrow(res)>1) {
      boxplot(y~xnew,main=paste(chem,"\n",celltype),
              ...
              horizontal=T,las=1,par(cex.lab=0.9,cex.axis=0.7),names=res$super_target)
