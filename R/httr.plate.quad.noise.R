#--------------------------------------------------------------------------------------
#' Look at quadrants of test plates to see if there is acheckerboard pattern
#' in the quality metrics
#'
#' @param to.file If TRUE, write plots to a file
#-------------------------------------------------------------------------------------
httr.plate.quad.noise <- function(to.file=F,set="mcf7",cutoff=1) {
  printCurrentFunction()
  if(to.file) {
    fname <- paste0("../httr/",set," httr.plate.quad.noise ",cutoff,".pdf")
  ...
  mat <- read.xlsx(file)
  mat <- mat[is.element(mat$stype,"test sample"),]
  mat <- mat[mat$conc<=cutoff,]
  npg <- 48
  ...
  for(pg in 1:npg) {
    ...
    for(plate in plate.list) {
      ...
      x <- NULL
      y <- NULL
      yy <- ptemp[is.element(ptemp$well_id,q1),"n_sig80"]
      xx <- yy
      xx[] <- "Q1"
      x <- c(x,xx)
      y <- c(y,yy)
      yy <- ptemp[is.element(ptemp$well_id,q2),"n_sig80"]
      xx <- yy
      xx[] <- "Q2"
      x <- c(x,xx)
      y <- c(y,yy)
      yy <- ptemp[is.element(ptemp$well_id,q3),"n_sig80"]
      xx <- yy
      xx[] <- "Q3"
      x <- c(x,xx)
      y <- c(y,yy)
      yy <- ptemp[is.element(ptemp$well_id,q4),"n_sig80"]
      xx <- yy
      xx[] <- "Q4"
      x <- c(x,xx)
      y <- c(y,yy)
      ...
      boxplot(y~x,main=paste("B,PG,P",block,pg,plate),ylim=c(ymin,ymax),
              cex.axis=1.2,cex.lab=1.2,ylab="n_sig80",xlab="Quadrant")
    }
    if(!to.file) browser()
  }
  if(to.file) dev.off()
