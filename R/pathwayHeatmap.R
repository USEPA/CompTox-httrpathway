...
#-------------------------------------------------------------------------------------
pathwayHeatmap <- function(to.file=F,
  ...
  if(to.file) {
    file <- paste0("../output/pathway_conc_resp_summary/PATHWAY_CR_",pathset,"_",dataset,"_",method,"_", pval,"_",nametag,".pdf")
    pdf(file=file,width=7,height=7,pointsize=12,bg="white",paper="letter",pagecentre=T)
  }


  file <- paste0("../output/pathway_conc_resp_summary/PATHWAY_CR_",pathset,"_",dataset,"_",method,"_", pval,"_", nametag ,".RData")
  print(file)
  load(file)
  mat <- PATHWAY_CR

  temp <- mat[,c("name","pathway","bmdl")]
  x <- temp$bmdl
  x[is.na(x)] <- 1000
  x <- 3-log10(x)
  x[mat$hitcall<0.5] <- 0
  temp$bmdl <- x
  res <- reshape2::dcast(temp,name~pathway)
  rownames(res) <- res[,1]
  res <- res[,2:ncol(res)]
