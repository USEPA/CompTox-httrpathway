...
#--------------------------------------------------------------------------------------
signatureTrends <- function(to.file=F,
                            do.load=F,
                            do.1=F,
                            do.2=F,
                            do.3a=F,
  ...
  if(to.file) {
    fname <- paste0("../output/signature_trends/signature_trends_",celltype,
                    "_",dataset,"_",sigset,"_",method,"_",hccut,".pdf")
  ...
  #mat <- mat[mat$bmd<maxbmd,]
  ##################################################################################
  if(do.1) {
    cat("fraction of hits per super target\n")
    st.list <- sort(unique(mat$super_target))
    res <- as.data.frame(matrix(nrow=length(st.list),ncol=2))
    names(res) <- c("super_target","fraction_hit")
    res[,1] <- st.list
    for(i in 1:length(st.list)) {
      st <- st.list[i]
      n0 <- nrow(mat0[is.element(mat0$super_target,st),])
      n1 <- nrow(mat[is.element(mat$super_target,st),])
      f = n1/n0
      res[i,"fraction_hit"] <- f
      if(f>0.1) cat(st,format(f,digits=2),"\n")
    }
    file <- paste0("../output/signature_trends/super_target_frac_hit_",celltype,
                   "_",dataset,"_",sigset,"_",method,"_",hccut,".xlsx")
    write.xlsx(res,file)
  }
  ##################################################################################
  if(do.2) {
    cat("fraction of hits per signature\n")
    st.list <- sort(unique(mat$signature))
    res <- as.data.frame(matrix(nrow=length(st.list),ncol=3))
    names(res) <- c("signature","super_target","fraction_hit")
    res[,1] <- st.list
    for(i in 1:length(st.list)) {
      st <- st.list[i]
      if(i==1) {
        temp0 <- mat0[is.element(mat0$signature,st),]
        n0 <- nrow(temp0)
      }
      temp1 <- mat[is.element(mat$signature,st),]
      n1 <- nrow(temp1)
      res[i,"super_target"] <- temp1[1,"super_target"]
      f = n1/n0
      res[i,"fraction_hit"] <- f
      if(f>0.1) cat(st,format(f,digits=2),"...",n0,n1,"\n")

    }
    file <- paste0("../output/signature_trends/signature_frac_hit_",celltype,
                   "_",dataset,"_",sigset,"_",method,"_",hccut,".xlsx")
    write.xlsx(res,file)
  }
  ##################################################################################
  if(do.3a) {
    cat("signature x chemical matrix with many hits\n")
    mat1 <- mat[mat$bmd<1,]
    #mat1 <- mat1[mat1$top_over_cutoff>2,]
    #mat1 <- mat1[mat1$ci<0.1,]
    cat("nrow mat:",nrow(mat1),"\n")
    cat("Chemicals:",length(unique(mat1$name)),"\n")
    cat("Signatures:",length(unique(mat1$signature)),"\n")
    res <- reshape2::dcast(mat1,name~signature,fill=0,value.var="auc",fun.aggregate=mean)
    rownames(res) <- res[,1]
  ...
  if(do.3b) {
    ...
    sig.tree <- hclust(dist.sig,method="average")#ward.D")
    chem.tree <- hclust(dist.chem,method="average")
    ...
    file <- paste0("../output/signature_trends/signature_cluster_",dataset,"_",sigset,"_",method,"_chems.xlsx")
    ...
    file <- paste0("../output/signature_trends/signature_cluster_",dataset,"_",sigset,"_",method,"_signature.xlsx")
  ...
  hist(mat0$hitcall,breaks=breaks,xlab="Hitcall",main=paste(celltype,"Hitcall: All"),cex.axis=1.2,cex.lab=1.2)
  breaks <- seq(from=0.9,to=1,by=0.01)
  hist(mat$hitcall,breaks=breaks,xlab="Hitcall",main=paste(celltype,"Hitcall: Filtered"),cex.axis=1.2,cex.lab=1.2)

  # T/C
  breaks <- seq(from=0,to=10,by=0.5)
  tc <- mat0$top_over_cutoff
  tc[tc>10] <- 10
  hist(tc,xlab="T/C",breaks=breaks,main=paste(celltype,"T/C: All"),cex.axis=1.2,cex.lab=1.2)
  breaks <- seq(from=0,to=10,by=0.5)
  tc <- mat$top_over_cutoff
  tc[tc>10] <- 10
  hist(tc,xlab="T/C",breaks=breaks,main=paste(celltype,"T/C: Filtered"),cex.axis=1.2,cex.lab=1.2)
  ...
  bmd[bmd< -3] <- -3
  bmd[bmd>3] <- 3
  hist(bmd,xlab="log(BMD)",breaks=breaks,main=paste(celltype,"log(BMD): All"),cex.axis=1.2,cex.lab=1.2)
  breaks <- seq(from=-3,to=3,by=0.2)
  bmd <- log10(mat$bmd)
  bmd[bmd< -3] <- -3
  bmd[bmd>3] <- 3
  hist(bmd,xlab="log(BMD)",breaks=breaks,main=paste(celltype,"log(BMD): Filtered"),cex.axis=1.2,cex.lab=1.2)

  # CI
  breaks <- seq(from=0,to=0.2,by=0.02)
  ci <- mat0$ci
  ci[ci>0.2] <- 0.2
  hist(ci,xlab="CI",breaks=breaks,main=paste(celltype,"CI: All"),cex.axis=1.2,cex.lab=1.2)
  breaks <- seq(from=0,to=0.2,by=0.02)
  ci <- mat$ci
  ci[ci>0.2] <- 0.2
  hist(ci,xlab="CI",breaks=breaks,main=paste(celltype,"CI: Filtered"),cex.axis=1.2,cex.lab=1.2)
  ...
  auc <- auc[!is.na(auc)]
  auc[auc>1] <- 1
  hist(auc,xlab="AUC",breaks=breaks,main=paste(celltype,"AUC: All"),cex.axis=1.2,cex.lab=1.2)
  breaks <- seq(from=0,to=1,by=0.1)
  auc <- mat$auc
  auc <- auc[!is.na(auc)]
  auc[auc>1] <- 1
  hist(auc,xlab="AUC",breaks=breaks,main=paste(celltype,"AUC: Filtered"),cex.axis=1.2,cex.lab=1.2)


  # sets of chemicals with high super target activity
  # promiscuous vs. specific super targets
  # fraction of super targets with few to no hits
  # chemicals with high ci
  # super targets with high ci
  # signatures with high ci
  # correlation between signatures
  # correlation between super targets
  #
  #
  if(!to.file) browser()
  else dev.off()
