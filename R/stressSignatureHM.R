...
stressPathwayHM <- function(to.file=F,
  ...
  for(sclass in sclass.list) {
    signature.list <- signature_catalog[is.element(signature_catalog$super_class,sclass),"signature"]
    gene.list <- NULL
    for(signature in signature.list) gene.list <- c(gene.list,signature_data[signature][[1]])
