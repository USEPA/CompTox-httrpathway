...
tc.trend.3 <- function(to.file=F,
  ...
  # Look at hte simple strends
  file <- "../input/PFAS info.xlsx"
  info <- read.xlsx(file)


  for(nc in c(3,4,6,7,8)) {
  ...
  for(st in st.list) {
    ...
    if(nrow(temp)>0) {
      ...
      for(i in 1:nrow(temp)) {
        ...
        bmd <- temp[i,"bmd"]
        lbmd <- -log10(bmd/1000)
        score <- temp[i,"hitcall"] * temp[i,"top_over_cutoff"] * lbmd / denom
  ...
  file <- paste0("../output/pfas_httr_score ",dataset,"_",sigset,".xlsx")
