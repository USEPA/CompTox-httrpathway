...
exportDSSToxSample <- function(outfile="../input/DSSTox/DSSTox_sample_map.xlsx") {
  ...
  ")
  res <- runQuery(query,"ro_prod_dsstox") #not sure where it is defined
  # not sure where runQuery comes from
  res <- unique(res)
